...
uint64_t read64bits_dram(uint64_t paddr)
{
    ...
    uint64_t val = 0x0;
    val += ((uint64_t)mm[paddr + 0]) << 0; // up说 uint8_t 进行 超过8bit的位运算会有bug，但是我测试没有，所以他有个类型转换
...
{
    int n = 10;
    uint64_t *high = (uint64_t *)&mm[va2pa(reg.rsp)]; // 从物理地址中获取 rsp 栈顶(top) 的内容
    high = &high[n]; // 获取十个寄存器的内容
